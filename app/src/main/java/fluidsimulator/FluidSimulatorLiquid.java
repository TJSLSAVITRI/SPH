...
import fluidsimulator.gameobjects.Particle;
import fluidsimulator.gameobjects.Piece;
import fluidsimulator.gameobjects.Spring;
import fluidsimulator.gameobjects.fluid.SpatialTable;
import fluidsimulator.utils.Vector2;
...
//			camera3D.update();
	...
	public boolean keyUp(int keycode) {
		// K
		if (keycode == Keys.F1 && K < 2.0f) {
			K += 0.1f;
		} else if (keycode == Keys.NUM_1 && K > 0.004f) {
			K -= 0.1f;
			if (K < 0.004f)
				K = 0.004f;
		}
		...
		// SIGMA
		if (keycode == Keys.F2 && SIGMA < 50.0f)
			SIGMA += 2;
		else if (keycode == Keys.NUM_2 && SIGMA > 0)
			SIGMA -= 2;

		// DENSITY
		else if (keycode == Keys.F3 && P0 < 1000.0f)
			P0 += 100.0f;
		else if (keycode == Keys.NUM_3 && P0 > 100.0f)
			P0 -= 100.0f;

		// H
		else if (keycode == Keys.F4 && H < 200.0f) {
			H += 5.0f;
			H2 = H * H;
			ATTRACT_RANGE = H2 / 2;
			REPULSE_RANGE = H2 / 4;
		} else if (keycode == Keys.NUM_4 && H > 5.0f) {
			H -= 5.0f;
			H2 = H * H;
			ATTRACT_RANGE = H2 / 2;
			REPULSE_RANGE = H2 / 4;
		...
		// CELL_SIZE
		if (keycode == Keys.F5 && CELL_SIZE < 50) {
			CELL_SIZE += 1;
			particles.rehash();
		} else if (keycode == Keys.NUM_5 && CELL_SIZE > 1) {
			CELL_SIZE -= 1;
			particles.rehash();
		}

		// K_
		if (keycode == Keys.F6 && K_ < 10.0f) {
			K_ += 0.1f;
		} else if (keycode == Keys.NUM_6 && K_ > 0.1f) {
			K_ -= 0.1f;
		}

		// MAX_NEIGHBORS
		if (keycode == Keys.F7 && particles.MAX_NEIGHBORS < 500) {
			particles.MAX_NEIGHBORS += 5;
			qArray = new float[particles.MAX_NEIGHBORS];
			qqArray = new float[particles.MAX_NEIGHBORS];
		} else if (keycode == Keys.NUM_7 && particles.MAX_NEIGHBORS > 0) {
			particles.MAX_NEIGHBORS -= 5;
			qArray = new float[particles.MAX_NEIGHBORS];
			qqArray = new float[particles.MAX_NEIGHBORS];
		}
		...
		// dropRadiusK
		if (keycode == Keys.F8 && dropRadiusK < 3.5f) {
			dropRadiusK += 0.2f;
			dropRadius = 0.1f + dropRadiusK;
			dropRadiusPixel = (int) (dropRadius * PARTICLE_SIZE);
			dropRadiusPixel2 = dropRadiusPixel * dropRadiusPixel;
			dropSprite.setSize(dropRadiusPixel, dropRadiusPixel);
		} else if (keycode == Keys.NUM_8 && dropRadiusK >= 0.3f) {
			dropRadiusK -= 0.2f;
			dropRadius = 0.1f + dropRadiusK;
			dropRadiusPixel = (int) (dropRadius * PARTICLE_SIZE);
			dropRadiusPixel2 = dropRadiusPixel * dropRadiusPixel;
		...
		// K_SPRING
		if (keycode == Keys.F9 && K_SPRING < 10.0f)
			K_SPRING += 0.2f;
		else if (keycode == Keys.NUM_9 && K_SPRING > 0.2f)
			K_SPRING -= 0.2f;
		
		// K_STICKINESS
		if (keycode == Keys.F10 && K_STICKINESS < 20.0f)
			K_STICKINESS += 1;
		else if (keycode == Keys.NUM_0 && K_STICKINESS > 0)
			K_STICKINESS -= 1;
		
		// RAD
		if (keycode == Keys.RIGHT && RAD < 50.0f) {
			RAD += 1;
			MULTIPLIER = 50 / RAD;
		}
		else if (keycode == Keys.LEFT && RAD > 0) {
			RAD -= 1;
			MULTIPLIER = 50 / RAD;
		}
		
		// VISC
		if (keycode == Keys.F11 && VISC < 1.0f)
			VISC += 0.0001f;
		else if (keycode == Keys.F12 && VISC > 0)
			VISC -= 0.0001f;

		if (keycode == Keys.BACKSPACE && isAttracting) {
			for (Particle pi : particles) {
				if (pi.pos.dst2(testPoint2D) < ATTRACT_RANGE) {
					disposableParticles.add(pi);
				}
			}
		} else if (keycode == Keys.BACKSPACE && IS_DESKTOP) {
			for (Particle pi : particles)
				disposableParticles.add(pi);
		}
		...
		if ((keycode == Keys.PLUS) && (GRAVITY_FORCE > -60.0f)) {
			GRAVITY_FORCE -= 1.0f;
			gravityVect.set(0.0f, GRAVITY_FORCE);
		} else if ((keycode == Keys.MINUS) && (GRAVITY_FORCE < 0.0f)) {
			GRAVITY_FORCE += 1.0f;
...
//				GRAVITY_FORCE = 0.0f;
		...
		if ((keycode == Keys.DOWN) && (EMITTER_SIZE > 2)) {
			EMITTER_SIZE -= 3;
		} else if ((keycode == Keys.UP) && (EMITTER_SIZE < 20)) {
			EMITTER_SIZE += 3;
		}
...
//			crazyMode = !crazyMode;
		...
		if (keycode == Keys.P && !viscoElasticityEnabled && IS_DESKTOP) {
		...
		if (keycode == Keys.V && !plasticityEnabled && IS_DESKTOP) {
