...
public class Renderer20 implements ImmediateModeRenderer {
	...
	public Renderer20 (boolean hasNormals, boolean hasColors, int numTexCoords) {
		this(5000, hasNormals, hasColors, numTexCoords, createDefaultShader(hasNormals, hasColors, numTexCoords));
		ownsShader = true;
	}

	public Renderer20 (int maxVertices, boolean hasNormals, boolean hasColors, int numTexCoords) {
	...
	public void end () {
		...
		shader.setUniformi("glossMap", 2);
		shader.setUniformi("displacementMap", 3);
		shader.setUniformi("displacementMap2", 4);
		shader.setUniformf("ScreenResX", Gdx.graphics.getWidth());
		shader.setUniformf("ScreenResY", Gdx.graphics.getHeight());
//		time += Gdx.graphics.getDeltaTime() * 0.03f * timeDir;
//		if (time > 0.1f || time < -0.1f)
//			timeDir *= -1;
		time += Gdx.graphics.getDeltaTime();
		shader.setUniformf("deltaTime", Gdx.graphics.getDeltaTime());
		shader.setUniformf("time", time);
		if (factor > 0.01f)
			factor = 0.01f;
		else if (factor < 0.001f)
				factor = 0.001f;
		shader.setUniformf("rippleIntensity", factor);
