...
import fluidsimulator.gameobjects.SpatialTable;
import fluidsimulator.pbf.Box;
...
//			camera3D.update();
	...
	public boolean keyUp(int keycode) {
		...
		// DENSITY
			...
			P0 -= 100.0f;
		if (keycode == Keys.F3 && REST_DENSITY < 100.0f)
			REST_DENSITY += 0.1f;
		if (keycode == Keys.NUM_3 && REST_DENSITY > 0.1f)
			REST_DENSITY -= 0.1f;

		// H
		if (keycode == Keys.F4 && H < 200.0f) {
			H += 5.0f;
			H2 = H * H;
			ATTRACT_RANGE = H2 / 2;
			REPULSE_RANGE = H2 / 4;
		} else if (keycode == Keys.NUM_4 && H > 0.2f) {
			H -= 5.0f;
			H2 = H * H;
			ATTRACT_RANGE = H2 / 2;
			REPULSE_RANGE = H2 / 4;
