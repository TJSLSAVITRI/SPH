...
public class FluidSimulatorMPM extends FluidSimulatorGeneric {
	...
	private float GRAVITY_FORCE =  -50.0f;
...
//				GRAVITY_FORCE = 0.0f;
		...
		} else if (keycode == Keys.NUM_1 && mpmSolver.materials.get(0).mass > 0) {
			mpmSolver.materials.get(0).mass -= 0.1f;
		}
		...
		else if (keycode == Keys.NUM_2 && mpmSolver.materials.get(0).restDensity > 0)
			mpmSolver.materials.get(0).restDensity -= 0.2f;

		// STIFFNESS
		else if (keycode == Keys.F3)
			mpmSolver.materials.get(0).stiffness += 0.1f;
		else if (keycode == Keys.NUM_3 && mpmSolver.materials.get(0).stiffness > 0f)
			mpmSolver.materials.get(0).stiffness -= 0.1f;

		// BULK VISCOSITY
		else if (keycode == Keys.F4) {
			mpmSolver.materials.get(0).bulkViscosity += 0.1f;
		} else if (keycode == Keys.NUM_4 && mpmSolver.materials.get(0).bulkViscosity > 0) {
			mpmSolver.materials.get(0).bulkViscosity -= 0.1f;
		}
		...
		} else if (keycode == Keys.NUM_5 && mpmSolver.materials.get(0).surfaceTension > 0) {
			mpmSolver.materials.get(0).surfaceTension -= 0.1f;
		}
		...
		} else if (keycode == Keys.NUM_6 && mpmSolver.materials.get(0).kElastic > 0) {
			mpmSolver.materials.get(0).kElastic -= 0.1f;
		}
		...
		} else if (keycode == Keys.NUM_7 && mpmSolver.materials.get(0).maxDeformation > 0) {
			mpmSolver.materials.get(0).maxDeformation -= 0.1f;
		}
		...
		} else if (keycode == Keys.NUM_8 && mpmSolver.materials.get(0).meltRate > 0) {
			mpmSolver.materials.get(0).meltRate -= 0.1f;
		}
		...
		else if (keycode == Keys.NUM_9 && mpmSolver.materials.get(0).viscosity > 0)
			mpmSolver.materials.get(0).viscosity -= 0.01f;
		
		// DAMPING
		if (keycode == Keys.F10)
			mpmSolver.materials.get(0).damping += 0.001f;
		else if (keycode == Keys.NUM_0 && mpmSolver.materials.get(0).damping > 0)
			mpmSolver.materials.get(0).damping -= 0.001f;
		
		// FRICTION
		if (keycode == Keys.F11)
			mpmSolver.materials.get(0).friction += 0.1f;
		else if (keycode == Keys.LEFT && mpmSolver.materials.get(0).friction > 0)
			mpmSolver.materials.get(0).friction -= 0.1f;
		
		// SMOOTHING
		if (keycode == Keys.F12)
			mpmSolver.materials.get(0).smoothing += 0.1f;
		else if (keycode == Keys.RIGHT && mpmSolver.materials.get(0).smoothing > 0)
			mpmSolver.materials.get(0).smoothing -= 0.1f;

		// GRAV
		if ((keycode == Keys.PLUS) && (mpmSolver.materials.get(0).gravity < 0.9f)) {
			mpmSolver.materials.get(0).gravity += 0.03f;
		} else if ((keycode == Keys.MINUS) && (mpmSolver.materials.get(0).gravity > 0.0f)) {
			mpmSolver.materials.get(0).gravity -= 0.03f;
